...
export default function Home(){
  ...
  const trustBookElement=useRef<HTMLDivElement>(null);
  const trustFlip=useRef<PageFlip|null>(null);
  ...
  const turnTrust=(direction:"next"|"prev")=>direction==="next"?trustFlip.current?.flipNext("top"):trustFlip.current?.flipPrev("top");
  ...
  useEffect(()=>{if(modal!=="trust"||!trustBookElement.current)return;const element=trustBookElement.current;const frame=requestAnimationFrame(()=>{const instance=new PageFlip(element,{width:960,height:540,size:"stretch",minWidth:280,maxWidth:900,minHeight:158,maxHeight:506,drawShadow:true,flippingTime:1150,usePortrait:true,startPage:activeTrust,autoSize:true,maxShadowOpacity:.42,showCover:true,mobileScrollSupport:false,swipeDistance:24,clickEventForward:true,useMouseEvents:true,disableFlipByClick:false});instance.on("flip",event=>setActiveTrust(Number(event.data)));instance.loadFromHTML(Array.from(element.querySelectorAll<HTMLElement>(".trust-flip-page")));trustFlip.current=instance});return()=>{cancelAnimationFrame(frame);trustFlip.current?.destroy();trustFlip.current=null}},[modal]);
  ...
  useEffect(()=>{const key=(e:KeyboardEvent)=>{if(e.key==="Escape")setModal(null);if(modal==="gallery"&&e.key==="ArrowRight")setSlide(s=>(s+1)%galleryTotal);if(modal==="gallery"&&e.key==="ArrowLeft")setSlide(s=>(s+galleryTotal-1)%galleryTotal);if(modal==="hospitality"&&activeHospitality===2&&e.key==="ArrowRight")setActiveReception(i=>(i+1)%receptionVideos.length);if(modal==="hospitality"&&activeHospitality===2&&e.key==="ArrowLeft")setActiveReception(i=>(i+receptionVideos.length-1)%receptionVideos.length);if(modal==="hospitality"&&activeHospitality===9&&e.key==="ArrowRight")setActiveControlObservation(i=>(i+1)%controlObservationSlideCount);if(modal==="hospitality"&&activeHospitality===9&&e.key==="ArrowLeft")setActiveControlObservation(i=>(i+controlObservationSlideCount-1)%controlObservationSlideCount);if(modal==="trust"&&e.key==="ArrowLeft")turnTrust("next");if(modal==="trust"&&e.key==="ArrowRight")turnTrust("prev");if(modal==="testimonial"&&e.key==="ArrowRight")setActiveTestimonial(i=>(i+1)%testimonials.length);if(modal==="testimonial"&&e.key==="ArrowLeft")setActiveTestimonial(i=>(i+testimonials.length-1)%testimonials.length)};addEventListener("keydown",key);return()=>removeEventListener("keydown",key)},[modal,galleryTotal,activeHospitality]);
  ...
  return <main className={`experience theme-${focus}`} dir={rtl?"rtl":"ltr"} onPointerMove={e=>{const el=e.currentTarget;el.style.setProperty("--mx",`${e.clientX}px`);el.style.setProperty("--my",`${e.clientY}px`)}}>
    ...
    {modal==="trust"&&<div className="trust-book-modal trust-pageflip-modal modal-layer" role="dialog" aria-modal="true" aria-label="شواهد الثقة" dir="rtl">
      ...
      <div className="trust-pageflip-stage">
        <div className="trust-pageflip-direction">
          <div ref={trustBookElement} className="trust-pageflip" aria-live="polite">
            {Array.from({length:trustSlideCount},(_,index)=><div className={`trust-flip-page ${index===0||index===trustSlideCount-1?"trust-flip-page--cover":""}`} data-density={index===0||index===trustSlideCount-1?"hard":"soft"} key={index}><img src={`${MEDIA_BASE}/media/trust/trust-slide-${index+1}.jpg`} alt={`شواهد الثقة - الصفحة ${index+1}`} draggable={false}/></div>)}
          </div>
        </div>
      </div>
      <div className="trust-book-nav trust-pageflip-nav"><button onClick={()=>turnTrust("prev")} aria-label={lang==="ar"?"الصفحات السابقة":"Previous pages"}><Chevron/></button><i><em style={{width:`${((activeTrust+1)/trustSlideCount)*100}%`}}/></i><b>{String(activeTrust+1).padStart(2,"0")} / {String(trustSlideCount).padStart(2,"0")}</b><button onClick={()=>turnTrust("next")} aria-label={lang==="ar"?"الصفحات التالية":"Next pages"}><Chevron/></button></div>
    </div>}
