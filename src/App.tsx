...
type Lang="ar"|"en"; type Level=1|2|3; type Modal="video"|"gallery"|"details"|"showcase"|"hospitality"|"trust"|"testimonial"|"testimonialVideo"|"comprehensive"|null; type Section="home"|"packages"|"testimonials"|"videos"|"team"|"hospitality";
const packs={
...
export default function Home(){
  ...
  useEffect(()=>{if(modal!=="comprehensive")return;const navigate=(event:KeyboardEvent)=>{if(event.key==="ArrowLeft"||event.key==="PageDown")setActiveComprehensive(index=>((index??-1)+1)%comprehensiveServices.length);if(event.key==="ArrowRight"||event.key==="PageUp")setActiveComprehensive(index=>((index??0)+comprehensiveServices.length-1)%comprehensiveServices.length)};addEventListener("keydown",navigate);return()=>removeEventListener("keydown",navigate)},[modal]);
    ...
    <section className={`hub-scene ${section==="home"?"is-here":""} ${activeHub!==null?"has-card-focus":""}`}>
      ...
      <div className="hub-carousel">
        <div className={`hub-options ${activeHub!==null?"has-active":""}`} ref={hubOptionsRef} onPointerLeave={()=>setActiveHub(null)}>
          ...
          <button className={activeHub===2?"is-active":""} onPointerEnter={()=>setActiveHub(2)} onFocus={()=>setActiveHub(2)} onClick={()=>transition(()=>setSection("team"))}><img src={`${MEDIA_BASE}/media/team/ali-bandaqji.webp`} alt=""/><span><TeamIcon/></span><strong>{t.team}</strong></button>
...
<span className="journey-node">{stage.kind==="video"?<Play/>:<Frames/>}</span><strong>{stage[lang]}</strong>
    ...
    {modal==="comprehensive"&&<div className="comprehensive-modal modal-layer" role="dialog" aria-modal="true" aria-label={lang==="ar"?"الباقة الشاملة":"Comprehensive Package"}>
      ...
      <div className="comprehensive-orbit" role="list" aria-label={lang==="ar"?"خدمات الباقة الشاملة":"Comprehensive package services"}>
        {comprehensiveServices.map((service,index)=><button type="button" role="listitem" key={service.icon} className={activeComprehensive===index?"is-active":""} style={{"--service-index":index} as React.CSSProperties} onClick={()=>setActiveComprehensive(index)} aria-pressed={activeComprehensive===index}><ComprehensiveServiceIcon kind={service.icon}/><span>{service[lang].title}</span></button>)}
      </div>
      {activeComprehensive!==null&&<aside className="comprehensive-panel" key={activeComprehensive} dir={rtl?"rtl":"ltr"}><button type="button" className="comprehensive-panel-close" onClick={()=>setActiveComprehensive(null)} aria-label={lang==="ar"?"إغلاق معلومات الخدمة":"Close service information"}>×</button><small dir="ltr">{String(activeComprehensive+1).padStart(2,"0")} / 06</small><h3>{comprehensiveServices[activeComprehensive][lang].title}</h3><p>{comprehensiveServices[activeComprehensive][lang].description}</p><ul>{comprehensiveServices[activeComprehensive][lang].points.map(point=><li key={point}>{point}</li>)}</ul></aside>}
    </div>}
