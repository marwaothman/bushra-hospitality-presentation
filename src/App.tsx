...
export default function Home(){
  ...
  useEffect(()=>{const key=(e:KeyboardEvent)=>{if(e.key==="Escape")setModal(null);if(modal==="gallery"&&e.key==="ArrowRight")setSlide(s=>(s+1)%galleryTotal);if(modal==="gallery"&&e.key==="ArrowLeft")setSlide(s=>(s+galleryTotal-1)%galleryTotal);if(modal==="hospitality"&&activeHospitality===2&&e.key==="ArrowRight")setActiveReception(i=>(i+1)%receptionVideos.length);if(modal==="hospitality"&&activeHospitality===2&&e.key==="ArrowLeft")setActiveReception(i=>(i+receptionVideos.length-1)%receptionVideos.length);if(modal==="hospitality"&&activeHospitality===9&&e.key==="ArrowRight")setActiveControlObservation(i=>(i+1)%controlObservationSlideCount);if(modal==="hospitality"&&activeHospitality===9&&e.key==="ArrowLeft")setActiveControlObservation(i=>(i+controlObservationSlideCount-1)%controlObservationSlideCount);if(modal==="trust"&&e.key==="ArrowRight")setActiveTrust(i=>(i+1)%trustSlideCount);if(modal==="trust"&&e.key==="ArrowLeft")setActiveTrust(i=>(i+trustSlideCount-1)%trustSlideCount);if(modal==="testimonial"&&e.key==="ArrowRight")setActiveTestimonial(i=>(i+1)%testimonials.length);if(modal==="testimonial"&&e.key==="ArrowLeft")setActiveTestimonial(i=>(i+testimonials.length-1)%testimonials.length)};addEventListener("keydown",key);return()=>removeEventListener("keydown",key)},[modal,galleryTotal,activeHospitality]);
