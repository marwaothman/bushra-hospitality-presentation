...
type Lang="ar"|"en"; type Level=1|2|3; type Modal="video"|"gallery"|"details"|"showcase"|"hospitality"|"trust"|"testimonial"|null; type Section="home"|"packages"|"testimonials"|"videos"|"team"|"hospitality";
const packs={
  ...
  return <main className={`experience theme-${focus}`} dir={rtl?"rtl":"ltr"} onPointerMove={e=>{const el=e.currentTarget;el.style.setProperty("--mx",`${e.clientX}px`);el.style.setProperty("--my",`${e.clientY}px`)}}>
    ...
    <section className={`testimonial-scene ${section==="testimonials"?"is-here":""}`} dir="rtl">
      {section==="testimonials"&&<div className="testimonial-stage">
        ...
        <div className="testimonial-grid">{testimonials.map((item,index)=><button type="button" className="testimonial-tile" key={item.name} onClick={()=>{setActiveTestimonial(index);setModal("testimonial")}}><span className="testimonial-mark">“</span><blockquote>«{item.quote}»</blockquote><span className="testimonial-person"><strong>{item.name}</strong><i>Facebook</i></span><span className="testimonial-thumb"><img src={item.image} alt=""/></span><span className="testimonial-open"><Arrow/></span></button>)}</div>
      </div>}
    </section>
