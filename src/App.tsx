...
export default function Home(){
  ...
  return <main key={refreshKey} className={`experience theme-${focus} mode-${viewMode??"select"}`} dir={rtl?"rtl":"ltr"} onErrorCapture={e=>{const target=e.target as HTMLElement;target.classList.add("media-failed");target.closest("figure,.pick-media,.showcase-poster,.leader-portrait,.video-screen-frame,.gallery-art")?.classList.add("has-media-fallback")}} onPointerMove={e=>{const el=e.currentTarget;el.style.setProperty("--mx",`${e.clientX}px`);el.style.setProperty("--my",`${e.clientY}px`);wakeControls()}} onTouchStart={e=>{presentationTouchStart.current=e.changedTouches[0].clientX;wakeControls()}} onTouchEnd={e=>{if(viewMode!=="guided"||modal==="gallery"||modal==="trust")return;const delta=e.changedTouches[0].clientX-presentationTouchStart.current;if(Math.abs(delta)>55)(delta<0?guidedNext:guidedPrevious)()}}>
    ...
    <section className={`closing-scene ${section==="closing"?"is-here":""}`} aria-labelledby="closing-title"><DrawLogo/><small>{lang==="ar"?"بُشرى لكل ضيف":"BUSHRA FOR EVERY GUEST"}</small><h2 id="closing-title">{lang==="ar"?"عناية تمتد عبر الرحلة… وأثر يبقى بعدها.":"Care throughout the journey. An impact that remains beyond it."}</h2><button type="button" onClick={()=>{setViewMode("explore");setSection("home");setAutoPlay(false)}}>{lang==="ar"?"إعادة العرض":"Restart presentation"}<Arrow/></button></section>
