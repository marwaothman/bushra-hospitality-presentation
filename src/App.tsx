...
<span className="journey-node">{stage.kind==="video"?<Play/>:<Frames/>}</span><strong>{stage[lang]}</strong>
    ...
    {modal==="hospitality"&&managedStages[activeHospitality]&&(activeHospitality<=1?<div className="cinema cinema--video hospitality-modal modal-layer" role="dialog" aria-modal="true" aria-label={activeHospitality===0?"بطاقة نسك لرحلة أيسر":managedStages[activeHospitality][lang]}><button className="x" onClick={()=>setModal(null)}>×<small>{t.close}</small></button><div className="cinema-video-bg" aria-hidden="true"><video src={`${MEDIA_BASE}/media/brand/background.mp4`} autoPlay muted loop playsInline/><div className="cinema-video-overlay"/></div><div className="video-screen-stage"><div className="video-screen-frame"><video key={`hospitality-${activeHospitality}`} className="services-video" src={activeHospitality===0?`${MEDIA_BASE}/media/hospitality/nusuk-card.mp4`:`${MEDIA_BASE}/media/hospitality/makkah-accommodation-v2.mp4`} poster={activeHospitality===0?`${MEDIA_BASE}/media/hospitality/nusuk-card-poster.jpg`:`${MEDIA_BASE}/media/hospitality/makkah-accommodation-v2-poster.jpg`} controls autoPlay playsInline preload="metadata">Your browser does not support video playback.</video><div className="video-vignette"/></div><h3 className="showcase-video-title">{activeHospitality===0?"بطاقة نسك لرحلة أيسر":managedStages[activeHospitality][lang]}</h3><span className="video-screen-shadow"/></div></div>:<div className="cinema hospitality-modal modal-layer" role="dialog" aria-modal="true" aria-label={managedStages[activeHospitality][lang]}><button className="x" onClick={()=>setModal(null)}>×<small>{t.close}</small></button><div className="cinema-rings"><i/><i/><i/></div><div className="hospitality-modal-copy"><span>{String(activeHospitality+1).padStart(2,"0")} / {managedStages.length}</span><div className="hospitality-modal-icon">{managedStages[activeHospitality].kind==="video"?<Play/>:<Frames/>}</div><h3>{managedStages[activeHospitality][lang]}</h3><p>{managedStages[activeHospitality].kind==="video"?t.videoMedia:t.galleryMedia} · {t.soon}</p></div></div>)}
