...
export default function Home(){
  const [lang,setLang]=useState<Lang>("ar"),[loading,setLoading]=useState(true),[section,setSection]=useState<Section>("home"),[selected,setSelected]=useState<Level|null>(null),[focus,setFocus]=useState<Level>(1),[modal,setModal]=useState<Modal>(null),[slide,setSlide]=useState(0);
  ...
  return <main className={`experience theme-${focus}`} dir={rtl?"rtl":"ltr"} onPointerMove={e=>{const el=e.currentTarget;el.style.setProperty("--mx",`${e.clientX}px`);el.style.setProperty("--my",`${e.clientY}px`)}}>
    ...
    <section className={`hub-scene ${section==="home"?"is-here":""}`}>
      ...
      <div className="hub-options">
        <button onClick={()=>transition(()=>setSection("packages"))}><span><PackagesIcon/></span><strong>{t.packages}</strong><em><Arrow/></em></button>
        <button onClick={()=>transition(()=>setSection("testimonials"))}><span><QuoteIcon/></span><strong>{t.testimonials}</strong><em><Arrow/></em></button>
        <button onClick={()=>transition(()=>setSection("videos"))}><span><FilmsIcon/></span><strong>{t.videos}</strong><em><Arrow/></em></button>
        <button onClick={()=>transition(()=>setSection("team"))}><span><TeamIcon/></span><strong>{t.team}</strong><em><Arrow/></em></button>
      </div>
    </section>
    ...
    <nav className={`page-step-nav global-step-nav ${modal?"is-obscured":""}`} aria-label={rtl?"التنقل بين الصفحات":"Page navigation"}>
      <button type="button" className="page-step page-step-prev" onClick={previousPage} disabled={atFirstPage} aria-label={rtl?"الصفحة السابقة":"Previous page"}><Arrow/></button>
      <span className="page-step-line"><i/></span>
      <button type="button" className="page-step page-step-next" onClick={nextPage} disabled={atLastPage} aria-label={rtl?"الصفحة التالية":"Next page"}><Arrow/></button>
    </nav>
