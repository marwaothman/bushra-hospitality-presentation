...
type Lang="ar"|"en"; type Level=1|2|3; type Modal="video"|"gallery"|"details"|"showcase"|"hospitality"|"trust"|"testimonial"|"testimonialVideo"|null; type Section="home"|"packages"|"testimonials"|"videos"|"team"|"hospitality";
const packs={
...
<span className="journey-node">{stage.kind==="video"?<Play/>:<Frames/>}</span><strong>{stage[lang]}</strong>
    ...
    {modal==="video"&&<div className={`cinema modal-layer ${selected?"cinema--video":""}`} role="dialog" aria-modal="true"><button className="x" onClick={()=>setModal(null)}>×<small>{t.close}</small></button>{selected?<><div className="cinema-video-bg" aria-hidden="true"><video src={`${MEDIA_BASE}/media/brand/background-video2.mp4`} autoPlay muted loop playsInline/><div className="cinema-video-overlay"/><span className="cinema-video-light cinema-video-light-a"/><span className="cinema-video-light cinema-video-light-b"/></div><div className="video-screen-stage"><div className="video-screen-frame"><video key={`services-${selected}`} className="services-video" src={`${MEDIA_BASE}/media/level-${selected}/services.mp4`} poster={`${MEDIA_BASE}/media/level-${selected}/services-poster.jpg`} controls autoPlay playsInline preload="metadata">Your browser does not support video playback.</video><div className="video-vignette"/></div><span className="video-screen-shadow"/></div></>:<><div className="cinema-rings"><i/><i/><i/></div><div className="cinema-copy"><span>PLAY FILM — 0{selected}</span><h3>{t.video}</h3><p>{t.videoNote}</p><button><Play/></button></div><div className="cinema-time">00:00 <i/> 02:45</div></>}</div>}
    {modal==="testimonial"&&<div className="testimonial-evidence-modal modal-layer" role="dialog" aria-modal="true" aria-label="الشهادة الأصلية" dir="rtl" onTouchStart={e=>touchStart.current=e.changedTouches[0].clientX} onTouchEnd={e=>{const delta=e.changedTouches[0].clientX-touchStart.current;if(Math.abs(delta)>45)setActiveTestimonial(i=>(i+(delta<0?1:testimonials.length-1))%testimonials.length)}}><button className="x" onClick={()=>setModal(null)}>×<small>{t.close}</small></button><div className="testimonial-evidence-stage"><div className="testimonial-evidence-copy"><span>شهادة موثقة</span><blockquote>«{testimonials[activeTestimonial].quote}»</blockquote><strong>{testimonials[activeTestimonial].name}</strong><a href={facebookPost} target="_blank" rel="noreferrer">عرض المنشور الأصلي <Arrow/></a></div><figure><img src={testimonials[activeTestimonial].image} alt={`تعليق ${testimonials[activeTestimonial].name} على فيسبوك`}/></figure></div><nav className="testimonial-evidence-nav"><button type="button" onClick={()=>setActiveTestimonial(i=>(i+testimonials.length-1)%testimonials.length)} aria-label="الشهادة السابقة"><Arrow/></button><b>{String(activeTestimonial+1).padStart(2,"0")} / {String(testimonials.length).padStart(2,"0")}</b><button type="button" onClick={()=>setActiveTestimonial(i=>(i+1)%testimonials.length)} aria-label="الشهادة التالية"><Arrow/></button></nav></div>}
    {modal==="testimonialVideo"&&<div className="cinema cinema--video showcase-modal modal-layer" role="dialog" aria-modal="true" aria-label={testimonialVideo[lang]}><button className="x" onClick={()=>setModal(null)}>×<small>{t.close}</small></button><div className="cinema-video-bg" aria-hidden="true"><video src={`${MEDIA_BASE}/media/brand/background-video2.mp4`} autoPlay muted loop playsInline/><div className="cinema-video-overlay"/></div><div className="video-screen-stage"><div className="video-screen-frame"><video className="services-video" src={testimonialVideo.src} poster={testimonialVideo.poster} controls autoPlay playsInline preload="metadata">Your browser does not support video playback.</video><div className="video-vignette"/></div><h3 className="showcase-video-title">{testimonialVideo[lang]}</h3><span className="video-screen-shadow"/></div></div>}\n    {modal==="gallery"&&<div className={`gallery modal-layer ${hasLevelOneArabic?"gallery--real":""} ${rtl?"gallery--rtl":"gallery--ltr"}`} role="dialog" aria-modal="true" onTouchStart={e=>touchStart.current=e.changedTouches[0].clientX} onTouchEnd={e=>{const delta=e.changedTouches[0].clientX-touchStart.current;if(Math.abs(delta)>45)setSlide(s=>(s+(delta<0?1:galleryTotal-1))%galleryTotal)}}>
      {hasLevelOneArabic&&<div className="gallery-video-bg" aria-hidden="true"><video className="video-panorama" src={`${MEDIA_BASE}/media/brand/background-video2.mp4`} autoPlay muted loop playsInline/><div className="video-overlay"/><span className="video-light video-light-a"/><span className="video-light video-light-b"/></div>}
    ...
    {modal==="showcase"&&<div className="cinema cinema--video showcase-modal modal-layer" role="dialog" aria-modal="true" aria-label={showcaseVideos[activeShowcase].title}><button className="x" onClick={()=>setModal(null)}>×<small>{t.close}</small></button><div className="cinema-video-bg" aria-hidden="true"><video src={`${MEDIA_BASE}/media/brand/background-video2.mp4`} autoPlay muted loop playsInline/><div className="cinema-video-overlay"/></div><div className="video-screen-stage"><div className="video-screen-frame"><video key={showcaseVideos[activeShowcase].src} className="services-video" src={showcaseVideos[activeShowcase].src} poster={showcaseVideos[activeShowcase].poster} controls autoPlay playsInline preload="metadata">Your browser does not support video playback.</video><div className="video-vignette"/></div><h3 className="showcase-video-title">{showcaseVideos[activeShowcase].title}</h3><span className="video-screen-shadow"/></div></div>}
    ...
    {modal==="trust"&&<div className="gallery gallery--real gallery--rtl trust-gallery modal-layer" role="dialog" aria-modal="true" aria-label="شواهد الثقة" dir="rtl" onTouchStart={e=>touchStart.current=e.changedTouches[0].clientX} onTouchEnd={e=>{const delta=e.changedTouches[0].clientX-touchStart.current;if(Math.abs(delta)>45)setActiveTrust(i=>(i+(delta<0?1:trustSlideCount-1))%trustSlideCount)}}>
      <div className="gallery-video-bg" aria-hidden="true"><video className="video-panorama" src={`${MEDIA_BASE}/media/brand/background-video2.mp4`} autoPlay muted loop playsInline/><div className="video-overlay"/><span className="video-light video-light-a"/><span className="video-light video-light-b"/></div>
