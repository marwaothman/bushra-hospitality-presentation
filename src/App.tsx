...
type Lang="ar"|"en"; type Level=1|2|3; type Modal="video"|"gallery"|"details"|"showcase"|"hospitality"|null; type Section="home"|"trust"|"packages"|"testimonials"|"videos"|"team"|"hospitality";
const packs={
...
export default function Home(){
  ...
  return <main className={`experience theme-${focus}`} dir={rtl?"rtl":"ltr"} onPointerMove={e=>{const el=e.currentTarget;el.style.setProperty("--mx",`${e.clientX}px`);el.style.setProperty("--my",`${e.clientY}px`)}}>
    ...
    <section className={`content-scene trust-scene ${section==="trust"?"is-here":""}`}>
      {section==="trust"&&<><div className="content-icon"><TrustIcon/></div><h2>{t.trustTitle}</h2><p>{t.soon}</p></>}
    </section>
    ...
    {modal==="hospitality"&&managedStages[activeHospitality]&&(activeHospitality===0?<div className="cinema cinema--video hospitality-modal modal-layer" role="dialog" aria-modal="true" aria-label="بطاقة نسك لرحلة أيسر"><button className="x" onClick={()=>setModal(null)}>×<small>{t.close}</small></button><div className="cinema-video-bg" aria-hidden="true"><video src={`${MEDIA_BASE}/media/brand/background.mp4`} autoPlay muted loop playsInline/><div className="cinema-video-overlay"/></div><div className="video-screen-stage"><div className="video-screen-frame"><video className="services-video" src={`${MEDIA_BASE}/media/hospitality/nusuk-card.mp4`} poster={`${MEDIA_BASE}/media/hospitality/nusuk-card-poster.jpg`} controls autoPlay playsInline preload="metadata">Your browser does not support video playback.</video><div className="video-vignette"/></div><h3 className="showcase-video-title">بطاقة نسك لرحلة أيسر</h3><span className="video-screen-shadow"/></div></div>:<div className="cinema hospitality-modal modal-layer" role="dialog" aria-modal="true" aria-label={managedStages[activeHospitality][lang]}><button className="x" onClick={()=>setModal(null)}>×<small>{t.close}</small></button><div className="cinema-rings"><i/><i/><i/></div><div className="hospitality-modal-copy"><span>{String(activeHospitality+1).padStart(2,"0")} / {managedStages.length}</span><div className="hospitality-modal-icon">{managedStages[activeHospitality].kind==="video"?<Play/>:<Frames/>}</div><h3>{managedStages[activeHospitality][lang]}</h3><p>{managedStages[activeHospitality].kind==="video"?t.videoMedia:t.galleryMedia} · {t.soon}</p></div></div>)}
