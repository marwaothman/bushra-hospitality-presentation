...
export default function Home(){
  ...
  const trustDestination=(current:number,direction:"next"|"prev")=>compactBook?(current+(direction==="next"?1:trustSlideCount-1))%trustSlideCount:direction==="next"?(current===0?1:current===1?3:current===3?5:0):(current===0?5:current===5?3:current===3?1:0);
  const turnTrust=(direction:"next"|"prev")=>{if(trustFlipping)return;setTrustTurn(direction);setTrustFlipping(true);window.setTimeout(()=>{setActiveTrust(current=>trustDestination(current,direction));setTrustFlipping(false)},860)};
  ...
  return <main className={`experience theme-${focus}`} dir={rtl?"rtl":"ltr"} onPointerMove={e=>{const el=e.currentTarget;el.style.setProperty("--mx",`${e.clientX}px`);el.style.setProperty("--my",`${e.clientY}px`)}}>
    ...
    {modal==="trust"&&<div className={`trust-book-modal modal-layer ${trustFlipping?"is-flipping":""}`} role="dialog" aria-modal="true" aria-label="شواهد الثقة" dir="rtl" data-turn={trustTurn} onTouchStart={e=>touchStart.current=e.changedTouches[0].clientX} onTouchEnd={e=>{const delta=e.changedTouches[0].clientX-touchStart.current;if(Math.abs(delta)>45)turnTrust(delta<0?"prev":"next")}}>
      ...
      <div className={`trust-book-stage ${activeTrust===0||activeTrust===trustSlideCount-1?"is-cover":"is-spread"}`}>
        <div className="trust-book" aria-live="polite">
          ...
          {trustFlipping&&<div className="trust-turning-sheet" aria-hidden="true"><div className="trust-turning-face trust-turning-face--front"><img src={`${MEDIA_BASE}/media/trust/trust-slide-${activeTrust+1}.jpg`} alt="" draggable={false}/></div><div className="trust-turning-face trust-turning-face--back"><img src={`${MEDIA_BASE}/media/trust/trust-slide-${trustDestination(activeTrust,trustTurn)+1}.jpg`} alt="" draggable={false}/></div><i/></div>}
