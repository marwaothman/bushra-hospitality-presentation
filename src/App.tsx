...
type Lang="ar"|"en"; type Level=1|2|3; type ViewMode="guided"|"explore"|null; type Modal="video"|"gallery"|"details"|"showcase"|"hospitality"|"trust"|"testimonial"|"testimonialVideo"|"comprehensive"|"qr"|null; type Section="home"|"impact"|"comparison"|"closing"|"packages"|"testimonials"|"videos"|"team"|"hospitality"; type ComprehensivePhase="idle"|"activating"|"panel";
const packs={
...
const comprehensiveServices=[
  {icon:"flight",ar:{title:"الطيران",description:"إدارة بداية الرحلة عبر تنسيق متكامل يربط مواعيد الوصول ببقية خدمات الباقة.",points:["تنسيق الرحلات","متابعة الوصول","ربط الخدمات التالية"]},en:{title:"Aviation",description:"An integrated start to the journey, connecting flight schedules with every service that follows.",points:["Flight coordination","Arrival monitoring","Connected onward services"]}},
  {icon:"transport",ar:{title:"النقل",description:"حركة منظمة وآمنة تربط المطار والفنادق والمشاعر ضمن خطة تشغيل موحدة.",points:["إدارة الحافلات","تخطيط المسارات","متابعة الحركة"]},en:{title:"Transportation",description:"Safe, coordinated movement connecting airports, hotels and the holy sites.",points:["Fleet management","Route planning","Live movement tracking"]}},
  {icon:"hotel",ar:{title:"الفنادق",description:"إقامة مدارة بعناية تضمن جاهزية الغرف وسهولة الوصول واستمرارية الخدمة.",points:["إدارة التسكين","جاهزية المرافق","خدمة الضيوف"]},en:{title:"Hotels",description:"Carefully managed accommodation supporting readiness, access and continuity of service.",points:["Room allocation","Facility readiness","Guest services"]}},
  {icon:"mashair",ar:{title:"المشاعر",description:"تجربة متكاملة في المشاعر تجمع الإقامة والخدمة والدعم الميداني.",points:["تهيئة المواقع","الدعم الميداني","استمرارية الخدمة"]},en:{title:"Holy Sites",description:"An integrated holy-sites experience combining accommodation, service and field support.",points:["Site readiness","Field support","Service continuity"]}},
  ...
  {icon:"care",ar:{title:"رضا الضيف",description:"إغلاق دائرة الخدمة عبر متابعة التجربة والاستجابة للملاحظات وتحسين جودة العناية.",points:["قياس التجربة","الاستجابة للملاحظات","التحسين المستمر"]},en:{title:"Guest Satisfaction",description:"Closing the service loop through experience monitoring, responsive support and continuous improvement.",points:["Experience measurement","Responsive support","Continuous improvement"]}},
] as const;
const packageProfiles={
  1:{ar:{need:"للبرامج التي تتطلب أعلى درجات التخصيص",care:"عناية مميزة",services:"إدارة متكاملة وخدمة ميدانية مخصصة",difference:"أعلى مستوى من المرونة والتخصيص",use:"الوفود والبرامج ذات المتطلبات الخاصة"},en:{need:"For programmes requiring the highest degree of customisation",care:"Signature care",services:"Integrated management and dedicated field service",difference:"Maximum flexibility and personalisation",use:"Delegations and programmes with special requirements"}},
  2:{ar:{need:"للبرامج المتوازنة بين التميز والكفاءة",care:"عناية متقدمة",services:"خدمات أساسية موسعة ومتابعة تشغيلية",difference:"توازن مدروس بين التجربة والكفاءة",use:"المجموعات المتوسطة والبرامج المتكاملة"},en:{need:"For programmes balancing distinction and efficiency",care:"Advanced care",services:"Expanded core services and operational follow-up",difference:"A considered balance of experience and efficiency",use:"Mid-size groups and integrated programmes"}},
  3:{ar:{need:"للبرامج التي تركز على الأساسيات الموثوقة",care:"عناية أساسية",services:"الخدمات الجوهرية بإدارة منظمة",difference:"وضوح تشغيلي وقيمة عملية",use:"البرامج واسعة النطاق ذات المتطلبات المحددة"},en:{need:"For programmes focused on dependable essentials",care:"Essential care",services:"Core services delivered through organised management",difference:"Operational clarity and practical value",use:"Large-scale programmes with defined requirements"}},
} as const;
const impactFacts={ar:[{value:"360°",label:"منظومة ضيافة مترابطة"},{value:"11",label:"قطاعاً في رحلة الضيف"},{value:"3",label:"مستويات للعناية"},{value:"4",label:"قيادات تنفيذية"}],en:[{value:"360°",label:"Connected hospitality ecosystem"},{value:"11",label:"Guest-journey sectors"},{value:"3",label:"Levels of care"},{value:"4",label:"Executive leaders"}]} as const;
...
export default function Home(){
  const [lang,setLang]=useState<Lang>("ar"),[loading,setLoading]=useState(true),[transitioning,setTransitioning]=useState(false),[viewMode,setViewMode]=useState<ViewMode>(null),[section,setSection]=useState<Section>("home"),[guidedIndex,setGuidedIndex]=useState(0),[autoPlay,setAutoPlay]=useState(false),[mediaPaused,setMediaPaused]=useState(false),[controlsActive,setControlsActive]=useState(true),[qrUrl,setQrUrl]=useState(""),[selected,setSelected]=useState<Level|null>(null),[focus,setFocus]=useState<Level>(1),[modal,setModal]=useState<Modal>(null),[slide,setSlide]=useState(0),[activeShowcase,setActiveShowcase]=useState(0),[activeHospitality,setActiveHospitality]=useState(0),[activeReception,setActiveReception]=useState(0),[activeTestimonial,setActiveTestimonial]=useState(0),[activeTrust,setActiveTrust]=useState(0),[activeComprehensive,setActiveComprehensive]=useState<number|null>(null),[comprehensivePhase,setComprehensivePhase]=useState<ComprehensivePhase>("idle"),[isFullscreen,setIsFullscreen]=useState(false),[activeHub,setActiveHub]=useState<number|null>(null);
  const [cms,setCms]=useState<CmsPayload|null>(null);
  const [refreshKey,setRefreshKey]=useState(0);
  const guidedSteps=["impact","trust","comprehensive","hospitality","comparison","packages","level1","level2","level3","testimonials","team","closing"] as const;
  const guidedTitles={ar:["بشرى في أرقام","شواهد الثقة","رحلة 360°","قطاعاتنا","مقارنة الباقات","مستويات العناية","المستوى الأول","المستوى الثاني","المستوى الثالث","شهادات من التجربة","فريقنا القيادي","بشرى لكل ضيف"],en:["Bushra in Numbers","Evidence of Trust","The 360° Journey","Our Sectors","Package Comparison","Levels of Care","Level One","Level Two","Level Three","Experience Stories","Leadership Team","Bushra for Every Guest"]} as const;
  ...
  const presentationTouchStart=useRef(0);
  ...
  useEffect(()=>{const video=backgroundVideoRef.current;if(!video)return;video.playbackRate=.72;if(modal||mediaPaused)video.pause();else video.play().catch(()=>{})},[modal,mediaPaused]);
  useEffect(()=>{document.querySelectorAll("video").forEach(video=>{if(mediaPaused)video.pause();else if(video===backgroundVideoRef.current&&!modal)video.play().catch(()=>{})})},[mediaPaused,modal]);
  useEffect(()=>{QRCode.toDataURL(location.href,{width:320,margin:2,color:{dark:"#133c67",light:"#ffffff"}}).then(setQrUrl).catch(()=>setQrUrl(""))},[]);
  useEffect(()=>{if("serviceWorker" in navigator)navigator.serviceWorker.register("./sw.js").catch(()=>{})},[]);
  useEffect(()=>{if(viewMode!=="guided")return;const next=guidedSteps[Math.min(guidedIndex+1,guidedSteps.length-1)];const sources:Partial<Record<typeof guidedSteps[number],string>>={trust:`${MEDIA_BASE}/media/trust/trust-slide-1.jpg`,comprehensive:"/media/brand/comprehensive-package-cover.webp",hospitality:`${MEDIA_BASE}/media/hospitality/makkah-accommodation-poster.jpg`,packages:`${MEDIA_BASE}/media/level-1/services-poster.jpg`,team:"/media/brand/leadership-cover-v2.jpg"};if(sources[next]){const image=new Image();image.src=sources[next]!}},[viewMode,guidedIndex]);
  ...
  useEffect(()=>()=>{transitionTimers.current.forEach(clearTimeout);if(comprehensiveTimer.current)clearTimeout(comprehensiveTimer.current);if(controlsTimer.current)clearTimeout(controlsTimer.current)},[]);
  useEffect(()=>{const key=(e:KeyboardEvent)=>{if(e.key==="Escape")setModal(null);if(modal==="gallery"&&e.key==="ArrowRight")setSlide(s=>(s+1)%galleryTotal);if(modal==="gallery"&&e.key==="ArrowLeft")setSlide(s=>(s+galleryTotal-1)%galleryTotal);if(modal==="hospitality"&&activeHospitality===2&&e.key==="ArrowRight")setActiveReception(i=>(i+1)%receptionVideos.length);if(modal==="hospitality"&&activeHospitality===2&&e.key==="ArrowLeft")setActiveReception(i=>(i+receptionVideos.length-1)%receptionVideos.length);if(modal==="trust"&&e.key==="ArrowRight")setActiveTrust(i=>(i+1)%trustSlideCount);if(modal==="trust"&&e.key==="ArrowLeft")setActiveTrust(i=>(i+trustSlideCount-1)%trustSlideCount);if(modal==="testimonial"&&e.key==="ArrowRight")setActiveTestimonial(i=>(i+1)%testimonials.length);if(modal==="testimonial"&&e.key==="ArrowLeft")setActiveTestimonial(i=>(i+testimonials.length-1)%testimonials.length)};addEventListener("keydown",key);return()=>removeEventListener("keydown",key)},[modal,galleryTotal,activeHospitality]);
  ...
  const openGuidedStep=(index:number)=>{const safe=Math.max(0,Math.min(guidedSteps.length-1,index));const step=guidedSteps[safe];setGuidedIndex(safe);setModal(null);setSelected(null);if(step==="trust"){setActiveTrust(0);setModal("trust");setSection("home")}else if(step==="comprehensive"){setActiveComprehensive(null);setComprehensivePhase("idle");setModal("comprehensive");setSection("home")}else if(step==="level1"||step==="level2"||step==="level3"){const level=Number(step.slice(-1)) as Level;setFocus(level);setSelected(level);setSection("packages")}else setSection(step as Section)};
  const startGuided=()=>transition(()=>{setViewMode("guided");openGuidedStep(0)});
  const startExplore=()=>transition(()=>{setViewMode("explore");setSection("home");setSelected(null);setModal(null)});
  const guidedNext=()=>transition(()=>openGuidedStep((guidedIndex+1)%guidedSteps.length));
  const guidedPrevious=()=>transition(()=>openGuidedStep((guidedIndex+guidedSteps.length-1)%guidedSteps.length));
  useEffect(()=>{if(!autoPlay||viewMode!=="guided")return;const id=window.setInterval(guidedNext,12000);return()=>clearInterval(id)},[autoPlay,viewMode,guidedIndex]);
  const wakeControls=()=>{setControlsActive(true);if(controlsTimer.current)clearTimeout(controlsTimer.current);controlsTimer.current=window.setTimeout(()=>setControlsActive(false),3200)};
  ...
  const previousPage=()=>transition(()=>{
    if(viewMode==="guided"){openGuidedStep((guidedIndex+guidedSteps.length-1)%guidedSteps.length);return}
    setModal(null);
  ...
  const nextPage=()=>transition(()=>{
    if(viewMode==="guided"){openGuidedStep((guidedIndex+1)%guidedSteps.length);return}
    setModal(null);
  ...
  useEffect(()=>{const key=(e:KeyboardEvent)=>{wakeControls();if(viewMode==="guided"&&(e.key==="ArrowRight"||e.key==="PageDown"||e.key===" ")){e.preventDefault();guidedNext();return}if(viewMode==="guided"&&(e.key==="ArrowLeft"||e.key==="PageUp")){e.preventDefault();guidedPrevious();return}if(modal)return;if(e.key==="PageDown"||e.key===" "){e.preventDefault();nextPage();return}if(e.key==="PageUp"){e.preventDefault();previousPage();return}if(section==="home"&&e.key==="ArrowRight"){hubOptionsRef.current?.scrollBy({left:hubOptionsRef.current.clientWidth*.72,behavior:"smooth"});return}if(section==="home"&&e.key==="ArrowLeft"){hubOptionsRef.current?.scrollBy({left:-hubOptionsRef.current.clientWidth*.72,behavior:"smooth"});return}if(e.key==="ArrowRight")nextPage();if(e.key==="ArrowLeft")previousPage()};addEventListener("keydown",key);return()=>removeEventListener("keydown",key)},[modal,section,viewMode,guidedIndex]);
  useEffect(()=>{if(section==="home")requestAnimationFrame(()=>hubOptionsRef.current?.scrollTo({left:0,behavior:"auto"}))},[section,lang]);
  return <main key={refreshKey} className={`experience theme-${focus} mode-${viewMode??"select"}`} dir={rtl?"rtl":"ltr"} onErrorCapture={e=>{const target=e.target as HTMLElement;target.classList.add("media-failed");target.closest("figure,.pick-media,.showcase-poster,.leader-portrait,.video-screen-frame,.gallery-art")?.classList.add("has-media-fallback")}} onPointerMove={e=>{const el=e.currentTarget;el.style.setProperty("--mx",`${e.clientX}px`);el.style.setProperty("--my",`${e.clientY}px`);wakeControls()}} onTouchStart={e=>{presentationTouchStart.current=e.changedTouches[0].clientX;wakeControls()}} onTouchEnd={e=>{if(viewMode!=="guided"||modal==="gallery"||modal==="trust")return;const delta=e.changedTouches[0].clientX-presentationTouchStart.current;if(Math.abs(delta)>55)(delta<0?guidedNext:guidedPrevious)()}}>
    ...
    {!loading&&viewMode===null&&<section className="mode-gateway" aria-labelledby="executive-positioning">
      <div className="gateway-copy"><small>{lang==="ar"?"بُشرى لكل ضيف":"BUSHRA FOR EVERY GUEST"}</small><h1 id="executive-positioning">{lang==="ar"?"منظومة ضيافة متكاملة ترافق ضيف الرحمن في كل مرحلة من رحلته.":"An integrated hospitality ecosystem serving the pilgrim throughout every stage of the journey."}</h1><i/></div>
      <div className="mode-options"><button type="button" onClick={startGuided}><span>01</span><strong>{lang==="ar"?"عرض موجّه":"Guided Presentation"}</strong><p>{lang==="ar"?"رحلة تنفيذية مرتبة للاجتماعات والعروض الرسمية.":"A structured executive journey for meetings and formal presentations."}</p><Arrow/></button><button type="button" onClick={startExplore}><span>02</span><strong>{lang==="ar"?"استكشاف حر":"Explore Freely"}</strong><p>{lang==="ar"?"الوصول المباشر إلى جميع الأقسام والمحتوى.":"Direct access to every section and experience."}</p><Arrow/></button></div>
    </section>}
    ...
    <section className={`impact-scene ${section==="impact"?"is-here":""}`} aria-labelledby="impact-title"><header><small>02 / IMPACT</small><h2 id="impact-title">{lang==="ar"?"بشرى في أرقام":"Bushra in Numbers"}</h2><p>{lang==="ar"?"حقائق موثقة من بنية العرض الحالي تعكس اتساع منظومة العناية.":"Verified facts from the current service architecture, expressing the breadth of Bushra's care ecosystem."}</p></header><div className="impact-grid">{impactFacts[lang].map((fact,index)=><article key={fact.label} style={{"--fact-index":index} as React.CSSProperties}><strong>{fact.value}</strong><span>{fact.label}</span></article>)}</div></section>

    <section className={`comparison-scene ${section==="comparison"?"is-here":""}`} aria-labelledby="comparison-title"><header><small>07 / PACKAGES</small><h2 id="comparison-title">{lang==="ar"?"مستويات عناية لكل احتياج":"A Care Level for Every Need"}</h2><p>{lang==="ar"?"مقارنة تنفيذية مختصرة تساعد متخذ القرار على اختيار التجربة الأنسب.":"A concise executive comparison designed to support confident decision-making."}</p></header><div className="comparison-grid">{([1,2,3] as Level[]).map(level=>{const profile=packageProfiles[level][lang];return <article key={level} className={`comparison-card comparison-card-${level}`}><div className="comparison-card-head"><small>0{level}</small><h3>{managedPacks[level][lang]}</h3><b>{profile.care}</b></div><dl><div><dt>{lang==="ar"?"الاحتياج":"Client need"}</dt><dd>{profile.need}</dd></div><div><dt>{lang==="ar"?"الخدمات":"Included services"}</dt><dd>{profile.services}</dd></div><div><dt>{lang==="ar"?"الميزة":"Key differentiator"}</dt><dd>{profile.difference}</dd></div><div><dt>{lang==="ar"?"الأنسب لـ":"Recommended for"}</dt><dd>{profile.use}</dd></div></dl><button type="button" onClick={()=>{setFocus(level);setSelected(level);setSection("packages")}}>{lang==="ar"?"استعرض المستوى":"Explore level"}<Arrow/></button></article>})}</div></section>

    <section className={`closing-scene ${section==="closing"?"is-here":""}`} aria-labelledby="closing-title"><DrawLogo/><small>{lang==="ar"?"بُشرى لكل ضيف":"BUSHRA FOR EVERY GUEST"}</small><h2 id="closing-title">{lang==="ar"?"عناية تمتد عبر الرحلة… وأثر يبقى بعدها.":"Care throughout the journey. An impact that remains beyond it."}</h2><button type="button" onClick={()=>{setViewMode(null);setSection("home");setAutoPlay(false)}}>{lang==="ar"?"إعادة العرض":"Restart presentation"}<Arrow/></button></section>
    ...
    <section className={`home-scene ${section!=="packages"||selected?"is-away":""}`}>
      ...
      <button type="button" className="comparison-entry" onClick={()=>transition(()=>setSection("comparison"))}>{lang==="ar"?"مقارنة الباقات":"Compare packages"}<Arrow/></button><div className="portal-stage">
    ...
    <section className={`hospitality-scene ${section==="hospitality"?"is-here":""}`} aria-labelledby="hospitality-title">
      ...
      <div className="journey-map" role="list">
        {managedStages.map((stage,index)=><button type="button" role="listitem" className="journey-stage" key={stage.ar} style={{"--stage-index":index} as React.CSSProperties} onClick={()=>{setActiveHospitality(index);if(index===2)setActiveReception(0);setModal("hospitality")}}>
<span className="journey-node">{stage.kind==="video"?<Play/>:<Frames/>}</span><strong>{stage[lang]}</strong>
    ...
    {modal==="video"&&<div className={`cinema modal-layer ${selected?"cinema--video":""}`} role="dialog" aria-modal="true">{selected?<><div className="cinema-video-bg" aria-hidden="true"><video src={`${MEDIA_BASE}/media/brand/background-video2.mp4`} poster={`${MEDIA_BASE}/media/brand/background-poster.jpg`} muted playsInline preload="metadata"/><div className="cinema-video-overlay"/><span className="cinema-video-light cinema-video-light-a"/><span className="cinema-video-light cinema-video-light-b"/></div><div className="video-screen-stage"><div className="video-screen-frame"><video key={`services-${selected}`} className="services-video" src={`${MEDIA_BASE}/media/level-${selected}/services.mp4`} poster={`${MEDIA_BASE}/media/level-${selected}/services-poster.jpg`} controls autoPlay playsInline preload="metadata">Your browser does not support video playback.</video><div className="video-vignette"/></div><span className="video-screen-shadow"/></div></>:<><div className="cinema-rings"><i/><i/><i/></div><div className="cinema-copy"><span>PLAY FILM — 0{selected}</span><h3>{t.video}</h3><p>{t.videoNote}</p><button><Play/></button></div><div className="cinema-time">00:00 <i/> 02:45</div></>}</div>}
    ...
    {modal==="comprehensive"&&<div className="comprehensive-modal modal-layer" role="dialog" aria-modal="true" aria-label={lang==="ar"?"الباقة الشاملة":"Comprehensive Package"} data-phase={comprehensivePhase} data-selected={activeComprehensive??"none"}>
      ...
      {activeComprehensive!==null&&comprehensivePhase==="panel"&&<aside className="comprehensive-panel" key={activeComprehensive} dir={rtl?"rtl":"ltr"} style={{"--selected-service":activeComprehensive} as React.CSSProperties}><button type="button" className="comprehensive-panel-close" onClick={closeComprehensiveService} aria-label={lang==="ar"?"إغلاق معلومات الخدمة":"Close service information"}>×</button><small dir="ltr">{String(activeComprehensive+1).padStart(2,"0")} / 07</small><h3>{comprehensiveServices[activeComprehensive][lang].title}</h3><p>{comprehensiveServices[activeComprehensive][lang].description}</p><ul>{comprehensiveServices[activeComprehensive][lang].points.map(point=><li key={point}>{point}</li>)}</ul></aside>}
    </div>}
    ...
    {viewMode&&<nav className={`presenter-controls ${controlsActive||modal?"is-active":""}`} aria-label={rtl?"أدوات مقدم العرض":"Presenter controls"}>
      <button type="button" className="presenter-arrow presenter-prev" onClick={previousPage} aria-label={rtl?"الفصل السابق":"Previous chapter"}><Chevron/></button>
      <div className="presenter-progress"><small>{viewMode==="guided"?`${String(guidedIndex+1).padStart(2,"0")} / ${guidedSteps.length}`:(lang==="ar"?"استكشاف حر":"FREE EXPLORE")}</small><strong>{viewMode==="guided"?guidedTitles[lang][guidedIndex]:(lang==="ar"?"اختر تجربتك":"Choose your experience")}</strong>{viewMode==="guided"&&<i><em style={{width:`${((guidedIndex+1)/guidedSteps.length)*100}%`}}/></i>}</div>
      <button type="button" className="presenter-arrow" onClick={nextPage} aria-label={rtl?"الفصل التالي":"Next chapter"}><Chevron/></button>
      <div className="presenter-utilities"><button type="button" onClick={()=>{setViewMode("explore");goHome()}} aria-label={rtl?"الرئيسية":"Home"}><HomeIcon/></button><button type="button" onClick={toggleFullscreen} aria-label={isFullscreen?(rtl?"الخروج من ملء الشاشة":"Exit full screen"):(rtl?"ملء الشاشة":"Full screen")}><FullscreenIcon active={isFullscreen}/></button><button type="button" className={autoPlay?"is-on":""} onClick={()=>setAutoPlay(value=>!value)} aria-label={rtl?"التشغيل التلقائي":"Automatic presentation"}>{autoPlay?"Ⅱ":"▶"}</button><button type="button" className={mediaPaused?"is-on":""} onClick={()=>setMediaPaused(value=>!value)} aria-label={rtl?"إيقاف الوسائط":"Pause media"}>{mediaPaused?"♪":"Ⅱ"}</button><button type="button" onClick={()=>setModal("qr")} aria-label={rtl?"رمز الاستجابة السريعة":"QR code"}>QR</button><button type="button" className="presenter-close" onClick={()=>modal?setModal(null):setViewMode(null)} aria-label={modal?t.close:(rtl?"اختيار وضع العرض":"Choose viewing mode")}>×</button></div>
    </nav>}
    {modal==="qr"&&<div className="qr-modal modal-layer" role="dialog" aria-modal="true"><div><small>{lang==="ar"?"افتح العرض على جهاز آخر":"OPEN ON ANOTHER DEVICE"}</small><h3>{lang==="ar"?"امسح الرمز للمتابعة":"Scan to continue"}</h3>{qrUrl&&<img src={qrUrl} alt={lang==="ar"?"رمز رابط العرض":"Presentation link QR code"}/>}<p dir="ltr">{location.href}</p></div></div>}
