...
export default function Home(){
  ...
  return <main key={refreshKey} className={`experience theme-${focus}`} dir={rtl?"rtl":"ltr"} onPointerMove={e=>{const el=e.currentTarget;el.style.setProperty("--mx",`${e.clientX}px`);el.style.setProperty("--my",`${e.clientY}px`)}}>
    ...
    {modal==="trust"&&<div className="trust-book-modal modal-layer" role="dialog" aria-modal="true" aria-label="شواهد الثقة" dir="rtl" data-turn={trustTurn} onTouchStart={e=>touchStart.current=e.changedTouches[0].clientX} onTouchEnd={e=>{const delta=e.changedTouches[0].clientX-touchStart.current;if(Math.abs(delta)>45)turnTrust(delta<0?"prev":"next")}}>
      ...
      <div className="trust-book-nav"><button onClick={()=>turnTrust("prev")} aria-label={lang==="ar"?"الصفحات السابقة":"Previous pages"}><Arrow/></button><i><em style={{width:`${((activeTrust+1)/trustSlideCount)*100}%`}}/></i><b>{!compactBook&&activeTrust>0&&activeTrust<trustSlideCount-1?`${String(activeTrust+1).padStart(2,"0")}–${String(activeTrust+2).padStart(2,"0")}`:String(activeTrust+1).padStart(2,"0")} / {String(trustSlideCount).padStart(2,"0")}</b><button onClick={()=>turnTrust("next")} aria-label={lang==="ar"?"الصفحات التالية":"Next pages"}><Arrow/></button></div>
    </div>}
