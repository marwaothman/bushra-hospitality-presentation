...
export default function Home(){
  ...
  return <main key={refreshKey} className={`experience theme-${focus}`} dir={rtl?"rtl":"ltr"} onPointerMove={e=>{const el=e.currentTarget;el.style.setProperty("--mx",`${e.clientX}px`);el.style.setProperty("--my",`${e.clientY}px`)}}>
...
<span className="journey-node">{stage.kind==="video"?<Play/>:<Frames/>}</span><strong>{stage[lang]}</strong>
    ...
    {modal==="video"&&<div className={`cinema modal-layer ${selected?"cinema--video":""}`} role="dialog" aria-modal="true"><button className="x" onClick={()=>setModal(null)}>×<small>{t.close}</small></button>{selected?<><div className="cinema-video-bg" aria-hidden="true"><video src={`${MEDIA_BASE}/media/brand/background-video2.mp4`} poster={`${MEDIA_BASE}/media/brand/background-poster.jpg`} muted playsInline preload="metadata"/><div className="cinema-video-overlay"/><span className="cinema-video-light cinema-video-light-a"/><span className="cinema-video-light cinema-video-light-b"/></div><div className="video-screen-stage"><div className="video-screen-frame"><video key={`services-${selected}`} className="services-video" src={`${MEDIA_BASE}/media/level-${selected}/services.mp4`} poster={`${MEDIA_BASE}/media/level-${selected}/services-poster.jpg`} controls autoPlay playsInline preload="metadata">Your browser does not support video playback.</video><div className="video-vignette"/></div><span className="video-screen-shadow"/></div></>:<><div className="cinema-rings"><i/><i/><i/></div><div className="cinema-copy"><span>PLAY FILM — 0{selected}</span><h3>{t.video}</h3><p>{t.videoNote}</p><button><Play/></button></div><div className="cinema-time">00:00 <i/> 02:45</div></>}</div>}
    ...
    {modal==="showcase"&&<div className="cinema cinema--video showcase-modal modal-layer" role="dialog" aria-modal="true" aria-label={showcaseVideos[activeShowcase].title}><button className="x" onClick={()=>setModal(null)}>×<small>{t.close}</small></button><div className="cinema-video-bg" aria-hidden="true"><video src={`${MEDIA_BASE}/media/brand/background-video2.mp4`} poster={`${MEDIA_BASE}/media/brand/background-poster.jpg`} muted playsInline preload="metadata"/><div className="cinema-video-overlay"/></div><div className="video-screen-stage"><div className="video-screen-frame"><video key={showcaseVideos[activeShowcase].src} className="services-video" src={showcaseVideos[activeShowcase].src} poster={showcaseVideos[activeShowcase].poster} controls autoPlay playsInline preload="metadata">Your browser does not support video playback.</video><div className="video-vignette"/></div><h3 className="showcase-video-title">{showcaseVideos[activeShowcase].title}</h3><span className="video-screen-shadow"/></div></div>}
